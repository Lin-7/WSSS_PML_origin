...
def load_image_label_list_from_npy(img_name_list, is_small):
    if is_small:
        cls_labels_dict = np.load('voc12/cls_labels_small.npy', allow_pickle=True).item()
    else:
        cls_labels_dict = np.load('voc12/cls_labels.npy',allow_pickle=True).item()
    # import pdb; pdb.set_trace()
    return [cls_labels_dict[img_name] for img_name in img_name_list]

def load_bounding_box_list_from_npy(img_name_list):
    bounding_box_dict = np.load('voc12/bounding_box.npy', allow_pickle=True).item()
    return [bounding_box_dict[str(img_name)] for img_name in img_name_list]

def get_img_path(img_name, voc12_root):
    return os.path.join(voc12_root, IMG_FOLDER_NAME, img_name + '.jpg')

def load_img_name_list(dataset_path, is_small):

    img_gt_name_list = open(dataset_path).read().splitlines()
    if is_small:
        img_name_list = [img_gt_name.split(' ')[0][-14:-4] for img_gt_name in img_gt_name_list]
    else:
...
class VOC12ClsDatasetMSF(VOC12ClsDataset):
    ...
    def __getitem__(self, idx):
        name, img, label,h, w = super().__getitem__(idx)
